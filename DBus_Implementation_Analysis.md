# tccd-ng DBus Interface Analysis

## Proper Implementation Based on tccd-ng Source Code Analysis

### Methods Analyzed

#### 1. **Display Brightness** ❌
- **Status**: NOT exposed via tccd-ng DBus
...
- **Status**: Partially exposed via tccd-ng DBus (read-only)
- **Available Methods**: 
  - `GetWebcamSWStatus()` ✅ - REGISTERED and IMPLEMENTED
  - `WebcamSWAvailable()` ✅ - REGISTERED
- **Missing Methods**:
  - `SetWebcamSWStatus()` ❌ - NOT registered in tccd-ng
- **Implementation**: 
...
#### 3. **Fan Data** ✅
- **Status**: FULLY exposed via tccd-ng DBus
- **Methods**: 
  - `GetFanDataCPU()` ✅
  - `GetFanDataGPU1()` ✅
  - `GetFanDataGPU2()` ✅
...
From `src/tccd-ng/TccDBusService.cpp` lines 250-310:

**Registered Methods in tccd-ng:**
```cpp
...
| Display Brightness | ❌ | ❌ | N/A | Not in tccd-ng |
| Fn Lock | ✅ | ✅ | Get/SetFnLockStatus | Working |

### Code Changes Made

Modified `ucc/libucc-dbus/TccdClient.cpp`:
...
3. **`setWebcamEnabled()`** - Returns `false` with comment explaining tccd-ng lacks setter
4. **`getWebcamEnabled()`** - Properly calls `GetWebcamSWStatus()` which IS registered

### Conclusion

✅ **All implementations now properly reflect what tccd-ng actually exposes:**
...
The application now correctly implements what the tccd-ng daemon provides, rather than attempting to call methods that don't exist.
