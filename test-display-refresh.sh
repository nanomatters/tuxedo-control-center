...
dbus-send --system --print-reply --dest=com.tuxedocomputers.tccd \
  /com/tuxedocomputers/tccd com.tuxedocomputers.tccd.GetIsX11
echo ""

# 2. Get display modes JSON
echo "2. Getting display modes (parsed):"
MODES=$(dbus-send --system --print-reply --dest=com.tuxedocomputers.tccd \
  /com/tuxedocomputers/tccd com.tuxedocomputers.tccd.GetDisplayModesJSON | \
  grep -oP '(?<=string ").*(?="$)')
...
PROFILE=$(dbus-send --system --print-reply --dest=com.tuxedocomputers.tccd \
  /com/tuxedocomputers/tccd com.tuxedocomputers.tccd.GetActiveProfileJSON | \
  grep -oP '(?<=string ").*(?="$)')
