...
**Current Status:** Code complete, blocked on build dependencies  
...
### CMake Structure
```
ucc/CMakeLists.txt           # Root config
...
### 🚧 Blocked
- [ ] **Build fails:** Missing qt6-qtdeclarative-devel package
  - Error: `Could NOT find Qt6Qml (missing: Qt6Qml_DIR)`
  - Location: `/usr/lib64/cmake/Qt6Qml/Qt6QmlConfig.cmake` does NOT exist
  - Issue: Fedora 44 (Rawhide) packaging inconsistency
  - `dnf` says package installed, `rpm` says it's not

### 🔄 Partially Complete
- [ ] TccdClient methods (50+ stubs return `std::nullopt`)
  - Need actual DBus method names from tccd-ng
  - Template pattern established, easy to fill in
- [ ] Plasma widgets DBus integration
  - Currently use mock data
  - Need to instantiate TccdClient in QML
...
- **Interface:** (needs introspection)

### Signal Examples (Qt Slots)
```cpp
// In TccdClient.cpp
...
### Project Location
`/opt/devel/git/tuxedo/tuxedo-control-center/ucc/`
...
**Status:** Awaiting dependency resolution to complete first build
